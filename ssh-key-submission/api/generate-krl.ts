...
const generateKRL = async (keys: string[], krlFilePath: string): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    exec(krlInitCommand, (error, stdout, stderr) => {
      ...
      const keyAdditionPromises = keys.map((key) => {
        ...
        return new Promise<void>((resolveKey, rejectKey) => {
...
async function fetchKeysFromDatabase(): Promise<string[]> {
  return [];
}
